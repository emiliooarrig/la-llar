...
export default function UnidadDetalle() {
  ...
  async function cargarUnidad() {
    ...
    try {
      ...
      const msg = e.response?.data?.error ?? 'No se pudo cargar la unidad.';
  ...
  async function handleSeleccionArchivo(e) {
    ...
    try {
      ...
      const res = await api.post(`/unidades/${id}/documentos`, formData, {
        headers: { 'Content-Type': undefined },
      });
      setUnidad(prev => ({
        ...prev,
        documentos_unidad: [res.data, ...(prev.documentos_unidad ?? [])],
      }));
  ...
  async function handleDescargar(doc) {
    try {
      const res = await api.get(`/unidades/documentos/${doc.id}/descargar`, {
        responseType: 'blob',
      });
  ...
  async function handleEstado(doc, nuevoEstado) {
    const textos = {
      ...
      desaprobado: { titulo: '¿Rechazar documento?',   texto: `"${doc.nombre_original}" quedará como rechazado.`,   color: '#D93025' },
    ...
    setCambiando(doc.id);
    try {
      const res = await api.patch(`/unidades/documentos/${doc.id}/estado`, { estado: nuevoEstado });
      setUnidad(prev => ({
        ...prev,
        documentos_unidad: prev.documentos_unidad.map(d =>
  ...
  async function handleEliminar(doc) {
    ...
    try {
      await api.delete(`/unidades/documentos/${doc.id}`);
      setUnidad(prev => ({
        ...prev,
        documentos_unidad: prev.documentos_unidad.filter(d => d.id !== doc.id),
      }));
    ...
    <div className={styles.pagina}>
      ...
      <main className={styles.contenido}>
        ...
        <div className={styles.navDetalle}>
          <button className={styles.btnVolver} onClick={() => navigate('/admin/unidades')}>
            <IconoFlecha /> Unidades
          </button>
          <span className={styles.breadcrumbSep}>/</span>
          <span className={styles.breadcrumbActual}>{unidad?.nombre ?? '…'}</span>
        </div>
        ...
        {cargando ? (
          ...
          <>
            ...
            <div className={styles.detalleHeader}>
              <div className={styles.detalleAvatar}>{unidad.nombre.charAt(0)}</div>
              <div className={styles.detalleInfo}>
                <p className={styles.detalleNombre}>{unidad.nombre}</p>
                <p className={styles.detalleRfc}>Cocina / Sucursal · {docs.length} documento{docs.length !== 1 ? 's' : ''}</p>
              </div>
            ...
            <div className={styles.tablaWrapper}>
                  ...
                  {docs.length === 0 ? (
                    <tr>
                      <td colSpan="4" className={styles.estadoTabla}>
                        Esta unidad no tiene documentos subidos aún.
                      </td>
                    </tr>
                  ) : (
                    docs.map(doc => (
                      <tr key={doc.id}>
                        ...
                        <td className={styles.colAcciones}>
                          <div className={styles.acciones}>
                            ...
                            {/* Rechazar (solo si no está desaprobado) */}
              ...
              {docs.length > 0 && (
                <p className={styles.pieTabla}>
                  ...
                  {docs.filter(d => d.estado === 'desaprobado').length} rechazado{docs.filter(d => d.estado === 'desaprobado').length !== 1 ? 's' : ''}
                </p>
              )}
            </div>
          </>
